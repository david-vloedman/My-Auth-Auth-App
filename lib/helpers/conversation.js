...
export const mapConversationToRedux = (
	...
	const alignedMessages = alignMessagesBySender(
...
const alignMessagesBySender = (messages, recipientObj) =>
	messages.map((msg) => ({
		...msg,
	...
	const intervalId = setInterval(async () => {
		try {
			...
			if (existingChat?.messages?.length < response?.data?.data?.length) {
				const alignedMessages = alignMessagesBySender(response.data.data, {
					id: recipientId,
					userName: recipientUserName,
				})
				dispatch(setMessages([...alignedMessages]))
