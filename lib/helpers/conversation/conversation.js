...
const loadConversation = (
	...
	console.log(	conversation.messages?.some(
		(msg) => !msg.hasBeenRead))

	if (
		conversation.messages?.some(
			(msg) => !msg.hasBeenRead && msg.sentBy === recipientId
		)
	)
...
const requestConversationByParticipants = async (
	dispatch,
	recipientId,
	recipientUserName
) => {
	try {
		const response = await axios.get(
			`/api/conversations/withUser/${recipientId}`
		)
...
export const openConversation = async (
	...
	try {
		// check application state and server for an existing conversation with this user
		const foundConversation =
			existingConversations.find((c) => c.participants.includes(recipientId)) ||
			(await requestConversationByParticipants(
				dispatch,
				recipientId,
				recipientUserName
			))
