 ...
 * @param {*} message 
 */
export const sendMessage = async (dispatch, conversationId, requestPostNewMessage, message) => {
	// dispatch message is sending...
	try{
		const response = await requestPostNewMessage(conversationId, message)
		// dispatch mess is done sending...
...
export const startUpdateInterval = async (
	...
	try {
		const intervalId = setInterval(async () => {
			...
			if (response) loadConversation(dispatch, response, recipientId, recipientUserName)
		}, 2000)
...
export const openConversation = async (
	...
	try {
		...
		startUpdateInterval(dispatch, conversation._id, requestConversation, recipientId, recipientUserName)
